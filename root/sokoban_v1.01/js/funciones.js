...
var aSkins = new Array();
	...
	aSkins[8] = ["Two Point Zero","img/twoPointZero/",16,16,true,"Alan Angelats","http://www.alanangelats.com"];
...
function inicializarNivel(){
	...
	if (nivel>numNiveles){
		alert("Enhorabuena!!!\nHa finalizado este grupo de niveles.")
		...
		if (estado==""){
			...
			if(confirm("Tiene una partida empezada.\n¿Desea continuarla?")){
				nNivelActual = nivel;
				aNivelActual = estado.split("|");
			}else if(confirm("¿Desea intentar el mismo nivel?")){
				reiniciarNivel();
			}else{
				reiniciarJuego();
			}
...
function escribirCreditos(){
	...
	if(aSkins[skin][5] && aSkins[skin][6]){
		document.write("&nbsp; &nbsp; <i>Skin: " + aSkins[skin][0] + " &copy <a href=\"" + aSkins[skin][6]+ "\">" + aSkins[skin][5] + "</a></i><br />");
	}
}
